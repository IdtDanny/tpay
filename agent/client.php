<?php
    ...
    $record_FetchQuery = 'SELECT * FROM `records` WHERE `rID` =:agentpin ORDER BY `rdate` AND `rtime` DESC';
    $record_FetchStatement = $pdo->prepare($record_FetchQuery);
    $record_FetchStatement->execute([ 'agentpin' => $agent_pin ]);
    $record_Result = $record_FetchStatement->fetch();

    $client_recorded_id = $record_Result->client_id;

    $client_FetchQuery = 'SELECT * FROM `client` WHERE `client_id` =:client_id ORDER BY `created_at` DESC';
    $client_FetchStatement = $pdo->prepare($client_FetchQuery);
    $client_FetchStatement->execute([ 'client_id' => $client_recorded_id ]);
